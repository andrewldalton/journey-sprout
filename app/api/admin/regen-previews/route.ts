/**
 * TEMPORARY admin route — regenerates the 4 story-picker preview images
 * server-side with a mix of page + cover renders so the picker feels varied.
 *
 * POST /api/admin/regen-previews
 *   Authorization: Bearer <TOKEN>
 *
 * Returns JSON with public Blob URLs keyed by final samples filename.
...
const TOKEN = "jsprout-preview-4b8e2f9d1c7a3e6f";
...
// Mix of pages + one cover so the picker feels like peeking into different
// moments, not four near-identical covers.
const JOBS: Job[] = [
  // 01 Moonbound — page 3 (bubble-helmet reveal)
  {
    kind: "page",
    pageNum: 3,
    storySlug: "01-night-stars",
    companionSlug: "sprig",
    samplesKey: "01-night-stars-cover.png",
  },
  // 02 Coral Dreams — keep as a cover (pebble/turtle)
  {
    kind: "cover",
    slug: "02-long-way-home",
    companionSlug: "pebble",
    samplesKey: "02-long-way-home-cover.png",
  },
  // 03 Zoo Day — page 4 (ballerina flamingo pirouette)
  {
    kind: "page",
    pageNum: 4,
    storySlug: "03-seed-took-time",
    companionSlug: "thistle",
    samplesKey: "03-seed-took-time-cover.png",
  },
  // 04 Jungle — page 8 (sparkly pool climax)
  {
    kind: "page",
    pageNum: 8,
    storySlug: "04-wish-already-had",
...
export async function POST(request: Request) {
  ...
  try {
      ...
      await time(label, async () => {
        if (job.kind === "cover") {
          ...
          const { url } = await uploadBytes(
            `admin-previews/${Date.now()}-${job.samplesKey}`,
            composed,
    ...
    console.error("[admin/regen-previews] failed", err);
    return Response.json(
      {
